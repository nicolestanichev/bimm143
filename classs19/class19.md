...
Pertissis is a bacterial lung infection also known as Whooping cough.
...
the mid 1940s lead to a dramatic reduction in case numbers (from over
200,000 to 100s).
...
The CMI-PB project is to prvide the scientific community with a
...
the database tables in a key:value type format like “infrancy_vac”:
“wP”. To read this we can use the `read_json()`function from the
**jsonlite** package. install with `install.packages("jsonlite")`
...
can begin to analyze this data and know who a give Ab sample was
...
Now let’s join the `ab_titer` table with out `meta` table so we have all
informations about a given Ab movement.
...
individuas. Levels peak at day 14 and ar larger in magnitude for wP than
aP individuals.
